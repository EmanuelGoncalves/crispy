...
master_lib["Library"] = pd.Categorical(master_lib["Library"], ["Avana", "KosukeYusa", "Brunello", "TKOv3"])
...
NGUIDES, JACKS_THRES, RULESET2_THRES, REMOVE_DISCORDANT = 2, 1.0, 0.4, False
...
for g in pgenes["symbol"]:
    ...
    # Top-up with KosukeYusa v1.1 guides
    if len(g_guides) < NGUIDES:
        g_guides_ky = select_guides(
            library="KosukeYusa",
            gene=g,
            offtarget=[1, 0],
        ...
        g_guides = pd.concat([g_guides, g_guides_ky], sort=False)
...
minimal_lib.to_csv(
    f"{DPATH}/crispr_libs/MinimalLib_top{NGUIDES}{'_disconcordant' if REMOVE_DISCORDANT else ''}_Avana.csv.gz",
    compression="gzip",
    index=False,
)

minimal_lib.to_excel(
    f"{RPATH}/MinimalLib_top{NGUIDES}{'_disconcordant' if REMOVE_DISCORDANT else ''}_Avana.xlsx",
    index=False,
)
