...
sns.violinplot('dummy', 'fc', 'Type', split=True, data=plot_df, palette={'All': cpal[0], 'Essential': cpal[1]}, linewidth=.3, cut=0, inner='quartile')
plt.title('MDA-MB-361')
plt.ylabel('Fold-change (log2)')
plt.xlabel('')
plt.gcf().set_size_inches(1, 2)
plt.savefig('reports/raw_counts_metric_violin.png', bbox_inches='tight', dpi=600)
plt.close('all')
