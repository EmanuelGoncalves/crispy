...
class CytidineBaseEditor(BaseEditor):
    def __init__(self, edit_window=(3, 8), pam=BaseEditor.PAM_WT_CAS9, len_guide=23, name='Cytidine base editor'):
        super().__init__(
...
class AdenineBaseEditor(BaseEditor):
    def __init__(self, edit_window=(3, 8), pam=BaseEditor.PAM_WT_CAS9, len_guide=23, name='Adenine base editor'):
        super().__init__(
            edit_window=edit_window,
            pam=pam,
            base_edit=('A', 'G'),
...
class BeditPlot(CrispyPlot):
    ...
    @classmethod
    def aa_grid(cls, dataframe=None, aggfunc=len, index_var='mutant', col_var='wildtype'):
        order = Modifications.get_order(field='abbreviation')

        plot_df = pd.pivot_table(
            dataframe[[index_var, col_var]], index=index_var, columns=col_var, aggfunc=aggfunc,
            fill_value=np.nan
        ).loc[order, order]

        #
        f, axs = plt.subplots(
            2, 2, sharex='col', sharey='row', gridspec_kw=dict(height_ratios=[1, 4], width_ratios=[4, 1])
        )

        g = sns.heatmap(
            plot_df, cmap='Greys', annot=True, cbar=False, linewidths=.5, ax=axs[1, 0]
        )

        axs[1, 0].set_xlabel('Wild-type')
        axs[1, 0].set_ylabel('Mutant')
        axs[1, 0].grid(lw=.5, ls='--', color=cls.PAL_DBGD[2])
        ...
        for i in [0, 1]:
            ...
            barfun(np.arange(len(order)) + .5, i_sums, color=colors)
    ...
    def aa_countplot(cls, dataframe, index_var='mutant', col_var='wildtype'):
        ...
        sns.barplot('count', 'aa', 'type', data=plot_df, palette=pal, order=order, orient='h')

        plt.grid(lw=.3, ls='-', color=cls.PAL_DBGD[2], axis='x', zorder=0)
