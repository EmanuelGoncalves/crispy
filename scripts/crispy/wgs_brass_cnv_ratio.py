...
# - Import
# Brass
brass_files = filter(lambda x: x.endswith('.brass.annot.bedpe'), os.listdir('data/gdsc/wgs/'))
brass_df = pd.concat([pd.read_csv('data/gdsc/wgs/%s' % f, skiprows=110, sep='\t') for f in brass_files]).reset_index(drop=True)
brass_df = brass_df.loc[list(map(lambda x: len(x.split(',')) == 1, brass_df['sample']))]

# Gene copy-number ratio
cnv_ratios = pd.read_csv('data/crispy_copy_number_ratio.csv', index_col=0).replace(-1, np.nan)
cnv_ratios.columns.name = 'sample'
...
# -
brass_msv = brass_df.groupby(['sample', 'gene1'])['svclass'].agg(lambda x: len(set(x)))
brass_msv = set(brass_msv[brass_msv > 1].index)

sv_ratio = pd.melt(brass_df.query("copynumber_flag == 1 & brass_score <= 100"), id_vars=['sample', 'svclass'], value_vars=['gene1', 'gene2']).query("value != '_'")
sv_ratio = sv_ratio[sv_ratio['value'].isin(genes) & sv_ratio['sample'].isin(samples)]

sv_ratio = sv_ratio.assign(ratio=[cnv_ratios.loc[g, s] for g, s in sv_ratio[['value', 'sample']].values]).dropna()
sv_ratio = sv_ratio.drop_duplicates(subset=['sample', 'svclass', 'value'])

sv_ratio = sv_ratio[[(s, g) not in brass_msv for s, g in sv_ratio[['sample', 'value']].values]]
...
ax.set_title('Structural variation relation\nwith copy-number ratio')
