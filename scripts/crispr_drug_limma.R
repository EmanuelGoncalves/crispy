...
# Import samplesheet
samplesheet <- read.csv('data/gdsc/crispr_drug/HT29_Dabraf_samplesheet.csv', row.names = 7)
samplesheet$id <- paste(samplesheet$medium, samplesheet$time, sep='_')

# Import data
data <- read.csv('data/gdsc/crispr_drug/HT29_dabraf_foldchanges_gene.csv', row.names = 1, check.names = F)
...
# Export results
res <- topTableF(fit2, adjust='BH', number=Inf)
write.csv(res, 'data/gdsc/crispr_drug/HT29_dabraf_foldchanges_gene_limma.csv', quote = F)
