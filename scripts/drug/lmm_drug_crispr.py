...
covs = pd.get_dummies(ss.loc[ys.index, ['Cancer Type', 'Microsatellite']]).astype(float)

# Linear regression
lmm = qtl_test_lmm(pheno=ys.values, snps=xs.values, K=k.values, covs=covs.values)
