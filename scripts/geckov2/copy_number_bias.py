...
if __name__ == '__main__':
    ...
    beds = gecko.import_crispy_beds()

    # - Add ceres gene scores to bed
    ceres = gecko.get_ceres()
    ...
    for y_label, y_var in [('original', 'fold_change'), ('corrected', 'corrected'), ('ceres', 'ceres')]:
        ...
        for x_var, x_thres in [('copy_number', 10), ('ratio', 4)]:
            ...
            for groupby, df in [('segment', beds_seg), ('gene', beds_gene)]:
                ...
                plt.savefig(f'reports/geckov2/bias_copynumber_{groupby}_{y_label}_{x_label}.png', bbox_inches='tight', dpi=600)
                plt.close('all')
    ...
    aucs_df.to_csv(f'{gecko.DIR}/bias_aucs.csv', index=False)
    ...
    for feature in ['copy_number', 'ratio']:
        for groupby in ['gene', 'segment']:
            ...
            plt.savefig(f'reports/geckov2/bias_pairgrid_{groupby}_{feature}.png', bbox_inches='tight', dpi=600)
            plt.close('all')
