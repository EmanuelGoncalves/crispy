...
control_label = "Plasmid_v1.1"
readcountfile = "/Users/eg14/Data/crispr_drug/RawData/Lib1.1/HT29_c907R2_D8.read_count.tsv"
...
normed = reads.ix[:, range(1, numColumns)] / np.tile(sumReads, [numClones, 1]) * 10000000       # normalize to 10M reads

#
# filter for minimum readcount
#
f = np.where(reads.ix[:, ctrl_column] >= MIN_READS)[0]
normed = normed.ix[f, :]
...
for i in range(numColumns - 1):
    foldchange[normed.columns.values[i]] = np.log2((normed.ix[:, normed.columns.values[i]] + 0.5) / (normed.ix[:, control_label] + 0.5))
#
